...
**Total de palavras:** total de palavras ditas por todos os personagens do filme.
**Total de personagens:** total de personagens do filme.
**Porcetagem de falas femininas:** Porcentagens de falas do filme que são de personagens femininas. 
...
Esse é o grupo dos filmes que possue o número total de palavras e o número total de personagens acima da média. Alguns exemplos mais conhecidos desse grupo são The Dark Knight, Se7en, The Wolf of Wall Street, The Godfather: Part II, que são filmes longos e com muitos personagens o que faz sentido uma vez que tais variáveis foram os motivos pelos quais esses filmes ficaram em um mesmo grupo.
...
p1 <- ggplot(filmes.km.long, aes(x = variable, y = value, group = title, colour = cluster)) +
...
É possível com essa visualização acima identificar alguns filmes presentes nos grupos como também alguns outliers em cada grupo. Pelo formato do comportamento das linhas fica claro como cada grupo tem filmes com características diferentes.
...
O tamanho das bolhas nesse gráfico é o número de personagens que um filme tem. Explore a visualização e tente encontrar filmes que você conhece e descubra informações sobre ele considerando as variáveis estudadas nessa análise.
