...
Para responder essa pergunta iremos determinar o intervalo de confiança para conseguirmos concluir algo sobre a diferença entre o grau de atividade dos programadores no meio de semana e no fim de semana. A medida estatística que utilizaremos será a mediana.
...
dados.go %>%
  ...
  theme(plot.title = element_text(hjust = 0.5))
```
Assim como em Java, Go também apresenta uma diferença visível de atividade entre o meio de semana e o final de semana. Como trata-se de uma amostra é preciso usar a inferência para poder dizer algo sobre a população e para isso utilizamos, novamente, bootstrap e intervalo de confiança.
...
data.frame(median.diff.ci) %>% 
  ...
  geom_hline(yintercept = 0, colour = "darkorange")
```
O critério utilizado para responder a pergunta foi determinar qual a diferença entre os programadores java e go no final de semana, será que uma linguagem é mais ativa no fim de semana do que a outra? E ainda, qual seria essa diferença?

A medida estatística utilizada foi a mediana, portanto estamos interessados em obter a diferença das medianas de usuários que editaram arquivos java e go nos finais de semana de janeiro de 2016 até maio de 2017.

Com 95% de confiança a diferença da medianas está no intervalo `r median.diff.ci`. Ou seja, a diferença é relevante e mostra que existem mais usuários ativos no final de semana programando em java do que em go.
