...
Algumas observações estão repetidas, para a análise essa repetição será retirada. Uma observação é considerada duplicada se possui a mesma url, ou seja, a mesma página de episódio no IMDB, que possui páginas individuais para cada episódio.
...
São `r nrow(series_netflix)` observações de 6 séries com `r ncol(series_netflix)` variáveis disponíveis.
...
A largura das barras foi colocada em um décimo para uma representação da distribuição mais real, uma vez que a diferença mínima entre os ratings que variam é de 1 décimo. Sobre a distribuição é possível notar que a grande maioria das notas dadas aos episódios estão acima de 8.
...
```{r}
summary(series_netflix$UserRating)
```
...
Outra visualização que nos diz muito, dessa vez sobre a variável do número de votos. Percebemos que a exceção da segunda temporada de Sense8, das demais séries, geralmente, possuem mais de 2000 votos, uma série se destaca das demais pelo alto número de votos, Stranger Things.

Outros sumários sobre o número de votos:

```{r}
summary(series_netflix$UserVotes)
```
...
Existem algumas possibilidades de determinar se uma série é melhor que outra ou não. Por exemplo, podemos definir que a média das notas de usuários de todos os episódios é um critério de forma que:
...
A partir dessa visualização passamos a ter uma visualização mais informações sobre os dados. Podemos observar que a mediana, linha central da caixa (boxplot) é superior em Sense8, no entanto, a **média (ponto em azul)** em Daredevil é maior. 13 Reasons Why, House of Cards e Narcos possuem medianas bem inferiores as demais e portanto já estão fora da disputa do prêmio de **série mais bem avaliada**. Sense8 é a série que possui maior variação nas notas atribuídas, ou seja, os episódios são avaliados com notas bem distintas, alguns são muito bem avaliados, outros muito mal. Esse fator faz com que essa série saia fora da disputa também. Uma forma de medir essa variação é determinar a diferença entre a nota mínima e a nota máxima de um episódio da série.
...
Definiremos iregularidade na qualidade dos episódios como os altos e baixos dos episódios dentro de uma temporada e entre temporadas.
...
series_netflix %>%
  ...
  labs(x = "Número do episódio", y = "Nota dos usuários", color = "Temporada")
```
Observando o gráfico notamos que em 13 reasons Why apesar de alguns episódios apresentarem uma queda com relaçao ao anterior, a tendência é que as notas atribuídas aumentem a medida que os episódios vão passando. De forma semelhante acontece em Stranger Things e em Sense8. Ou seja, se considerarmos irregularidade na qualidade como a falta de tendência clara de queda ou subida no rating dos usuários durante maior parte da temporada, séries como Narcos, Daredevil e House of Cards são as que apresentam maior irregularidade. Pois os episódios apresentam com bem mais frequência alternância entre altos e baixos.

Ao levarmos em conta as maiores variações e picos entre os episódios temos que House Of Cards possui *a maior irregularidade na qualidade dos episódios* seguida de perto por Narcos.
